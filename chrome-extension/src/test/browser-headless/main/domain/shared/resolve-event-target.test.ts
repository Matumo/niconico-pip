...
test.describe("domain共通 EventTarget解決", () => {
  ...
  test("ブラウザでEventTarget解決関数が動作すること", async () => {
    if (!environment) {
    ...
    try {
      ...
      const result = await executeHeadlessRuntimeTest(
        session.page,
        runtimeTestPathMap.main.domain.shared.resolveEventTargetTest,
      );
