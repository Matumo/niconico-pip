...
test.describe("debug dump trigger", () => {
  ...
  test("ブラウザでCustomEventによるdebug dump起動が動作すること", async () => {
    if (!environment) {
    ...
    try {
      ...
      const result = await executeHeadlessRuntimeTest(
        session.page,
        runtimeTestPathMap.main.debug.debugDumpTriggerTest,
      );
